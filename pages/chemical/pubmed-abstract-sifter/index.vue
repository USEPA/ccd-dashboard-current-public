    ...
    <mdb-container fluid class="content left-fullwidth-container">
      <div class="mr-4" style="margin-top:10px">
          ...
          >
            <div data-cy="accordionBody" style="font-size:12px;padding:10px">
              ...
              <p>
                ...
                Take a look at the blue pull-down box to the left (labeled Choose Query Term) and
                browse the selections there.  If you're coming to the Abstract Sifter via a chemical,
                we've supplied the chemical name, CASRN, and maybe a synonym in the query box.
                Selecting subject query terms in the pull-down will append that text to your chemical terms.
                Make any changes in the query box you need to before you submit by clicking on the blue button labeled
                Retrieve Articles.
              </p>

              <p>
                Once you click submit, the resulting citations from PubMed will be retrieved and
                inserted into the table below.  Now you can sift.
              </p>

              <p>
                Here's how to use the power of the Abstract Sifter by sifting:
                enter a term or part of a term into the first sifter box.
                The Sifter will automatically count the occurrences of that term in the title and abstract of the
                returned records and add a column to the table showing you that count.
                Use the built-in column header sorting functionality to sort the records.
                You can enter many sifter terms and change them as you explore the returned results.
                Sifting is a fast way to find an article of interest.
              </p>

              <p>
                To read an abstract, click on a row. The abstract along with the title will be displayed in the
                space below the table.  If the citation has sifter terms, they will be colorized for your reading enjoyment.
              </p>
              <p>
                Clicking on the hyperlink to PubMed will take you to the article there. Similarly,
                the DOI hyperlink will take you to the article at the journal site.
              </p>

              <p>
                If you would like to download a selection of citations, click on the button to the right
                labeled Send To.  Choose a download format option or choose to send the PubMed IDs to PubMed to
                take advantage of their download options.
              </p>

              <p>
                If you like this sifting approach to document retrieval but want to spend more time
                working with the results, try the Excel version of the Abstract Sifter.
                You can download it by clicking on About at the top of your screen, then on Downloads,
                then Abstract Sifter. The Sifter download file comes with a very detailed User Guide and the
                ReadMe sheet in the Sifter has links to videos and publications.
        ...
        <div class="toolbar mr-3 mb-1" data-cy="toolbarQuery">
          ...
          <div class="d-flex flex-row ">
            <div class="col-3 ">
              <select id="m1" v-model="s1" class="form-control" data-cy="dropdownQueryTermsMenu" @change="buildPhrase">
                <option
            ...
            <div class="col-4">
              <textarea
                ...
                aria-describedby="input textarea search query"
...
export default {
  ...
  watch: {
    ...
    completedPhrase(newVal, oldVal) {
      ...
      this.selectedTerms = this.selectedTerms.filter(term => newVal.includes(this.getPhraseText(term.id).trim()));
    ...
    lastTermAdded: {
      ...
      handler(newTerm) {
        ...
        this.gridApi.forEachNodeAfterFilter((rowNode, index) => {
          ...
          const termsCount = terms
            .map(term => (matches ? matches.filter(match => match.toLowerCase() === term.toLowerCase()).length : 0));
  ...
  head() {
    return {
      title: 'Publications - Abstract Sifter',
      meta: [],
    };
  },
  ...
  methods: {
    ...
    removeTerm(term) {
      ...
      this.sqlOperators.forEach((operator) => {
        ...
        // use lowercase for edge case - user created case insensitive modifications
    ...
    buildPhrase(e) {
      ...
      const item = this.m1Items.find(option => option.id === Number.parseInt(e.target.value, 10));
      ...
      if (this.s1 !== 0 && !isTermSelected) {
        this.categoryPhrase = `AND ${this.getPhraseText(this.s1)} `;
      }

      this.completedPhrase += ` ${this.categoryPhrase}`;
      this.completedPhrase = this.completedPhrase.trim();
    },
